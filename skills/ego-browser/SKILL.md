...
- Single-element actions and required reads auto-wait. Unqualified raw CSS and raw `xpath=` locators are not strict and act on the first match by default; prefer `getBy*`, `loc=...`, or chained/filtered locators when ambiguity must fail loudly, especially for destructive actions. For zero matches, confirm load, active tab, and modal/overlay state before correcting the locator. For multiple matches, inspect `count()` / `allInnerTexts()`, narrow semantically or with `filter(...)`, and use `first()` / `nth()` only after confirming duplicates are legitimate.
