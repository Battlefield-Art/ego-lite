...
- `await page.url()` — asynchronously returns the current URL string. Always await it before reading or using the URL.
...
Single-element locator actions are strict and auto-wait for a unique match up to their timeout. On a zero match, first verify the page finished loading, the correct tab is selected, and no modal/overlay changes the DOM, then correct the locator. On multiple matches, keep strictness: inspect `count()`/`allInnerTexts()`, narrow with semantic options or `filter(...)`, and use `first()`/`nth()` only after confirming that duplicates are legitimate.
