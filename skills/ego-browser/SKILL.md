...
- Single-element actions and required reads auto-wait. Only `getBy*`, `loc=...`, and chained or filtered locators are strict and fail on multiple matches; raw CSS and `xpath=` selectors silently act on the first match, so use a strict form for anything destructive. For zero matches, confirm load, active tab, and modal/overlay state before correcting the locator. For multiple matches, inspect `count()` / `allInnerTexts()`, narrow semantically or with `filter(...)`, and use `first()` / `nth()` only after confirming duplicates are legitimate.
