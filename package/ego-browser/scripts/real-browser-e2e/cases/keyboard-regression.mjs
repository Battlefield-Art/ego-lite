export function keyboardRegressionCase() {
  return `
    ...
    await evaluate("document.querySelector('#email-input').value = 'old@example.com'; window.__fixtureState.valueEvents['email-input'] = []; return true;");
    await fill("#email-input", "marcus.hale@example.com", { timeout: 3000 });
    const emailValue = await evaluate("return document.querySelector('#email-input').value");
    assertEqual(emailValue, "marcus.hale@example.com", "fill replaces type=email value (Issue 1)");
    const emailEvents = await evaluate("return (window.__fixtureState.valueEvents['email-input'] || []).join(',')");
    ...
    await evaluate("document.querySelector('#number-input').value = '123'; return true;");
    await fill("#number-input", "456", { timeout: 3000 });
    const numberValue = await evaluate("return document.querySelector('#number-input').value");
    ...
    await fill('xpath=//input[@id="text-input"]', "via-xpath", { timeout: 3000 });
    ...
    await fill("#controlled-input", "hello-react", { timeout: 3000 });
    const controlledValue = await evaluate("return document.querySelector('#controlled-input').value");
    assertEqual(controlledValue, "hello-react", "fill persists value on a react-style controlled input");
    const controlledState = await evaluate("return document.querySelector('#controlled-state').textContent");
