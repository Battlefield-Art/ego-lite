...
export const adversarialCases = [
  ...
  },
  {
    ...
    body: homeCase(`
      assert(Number.isNaN(await js("NaN")), "js decodes NaN unserializable value");
      assertEqual(await js("Infinity"), Infinity, "js decodes positive Infinity");
      assertEqual(await js("-Infinity"), -Infinity, "js decodes negative Infinity");
      assert(Object.is(await js("-0"), -0), "js preserves negative zero");
      assert(
        (await js("12345678901234567890n")) === 12345678901234567890n,
  ...
  },
  {
    ...
    body: homeCase(`
      assertEqual(await js("Promise.resolve(42)"), 42, "js awaits promise values");
      assertEqual(await js("'return is inside a string'"), "return is inside a string", "js does not wrap return inside strings");
      assertEqual(await js("// return is inside a comment\\n1 + 4"), 5, "js does not wrap return inside comments");
      assertEqual(await js("const returned = 1; returned + 1"), 2, "js does not treat returned as return keyword");
      await assertRejects(
        () => js("throw 'primitive boom'"),
        "primitive boom",
  ...
  },
  {
    ...
    body: homeCase(`
      await gotoAndWait(baseUrl + "/nav-target", { timeout: 10 });
      ...
      await gotoAndWait(baseUrl + "/", { timeout: 10 });
