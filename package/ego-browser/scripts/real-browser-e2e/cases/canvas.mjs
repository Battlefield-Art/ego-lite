...
/* Canvas drawing helper: returns a drawAt function that maps canvas-relative
   fractions (0–1) to viewport coordinates for dragMouse. This adapts to the
   actual rendered canvas size instead of assuming fixed 400×300. */
function canvasSetup() {
  return `
    const canvasRect = await js(\`(() => {
...
export const canvasCases = [
  {
    ...
    body: homeCase(`
      ...
      await dragMouse([drawAt(0.05, 0.5), drawAt(0.5, 0.5), drawAt(0.95, 0.5)], { delayMs: 10 });
      ...
      assertEqual(
        await js("return window.__fixtureState.canvasDrawing"),
        false,
        "canvas drawing state resets after mouseup"
      );

      /* Verify pixels were drawn at three positions along the stroke path. */
      assert(await js("return " + ${JSON.stringify(pixelDrawn(0.15, 0.5))}), "pixels drawn at stroke start region");
      assert(await js("return " + ${JSON.stringify(pixelDrawn(0.5, 0.5))}), "pixels drawn at stroke midpoint");
      assert(await js("return " + ${JSON.stringify(pixelDrawn(0.85, 0.5))}), "pixels drawn at stroke end region");

      /* Verify the last recorded point is near the end of the stroke. */
      const lastPoint = await js("return window.__fixtureState.canvasLastPoint");
      const lastFrac = lastPoint.x / await js("return document.querySelector('#draw-canvas').width");
  ...
  },
  {
    ...
    body: homeCase(`
      ...
      await dragMouse([drawAt(0.1, 0.25), drawAt(0.5, 0.25), drawAt(0.9, 0.25)], { delayMs: 15 });
      await waitForJsValue("window.__fixtureState.canvasStrokes", 1, "first stroke registered");
      assert(await js("return " + ${JSON.stringify(pixelDrawn(0.5, 0.25))}), "pixels drawn on first stroke path");

      /* Stroke 2: horizontal line in the lower third. */
      await wait(0.1);
      await dragMouse([drawAt(0.1, 0.75), drawAt(0.5, 0.75), drawAt(0.9, 0.75)], { delayMs: 15 });
      ...
      const secondStrokeFound = await js(\`(() => {
        const c = document.querySelector("#draw-canvas");
        const ctx = c.getContext("2d");
      ...
      await wait(0.1);
      await dragMouse([drawAt(0.3, 0.1), drawAt(0.3, 0.5), drawAt(0.3, 0.9)], { delayMs: 15 });
      await waitForJsValue("window.__fixtureState.canvasStrokes", 3, "third stroke registered");
      assert(await js("return " + ${JSON.stringify(pixelDrawn(0.3, 0.5))}), "pixels drawn on vertical stroke");
    `),
  },
  {
    ...
    body: homeCase(`
      ...
      assert(await js("return " + ${JSON.stringify(pixelDrawn(0.08, 0.5))}), "pixels at zigzag start");
      assert(await js("return " + ${JSON.stringify(pixelDrawn(0.3, 0.17))}), "pixels at zigzag peak 1");
      assert(await js("return " + ${JSON.stringify(pixelDrawn(0.5, 0.83))}), "pixels at zigzag valley");
      assert(await js("return " + ${JSON.stringify(pixelDrawn(0.75, 0.17))}), "pixels at zigzag peak 2");
      assert(await js("return " + ${JSON.stringify(pixelDrawn(0.93, 0.5))}), "pixels at zigzag end");
