...
export const canvasCases = [
  {
    ...
    body: homeCase(`
      ...
      await drag([drawAt(0.05, 0.5), drawAt(0.5, 0.5), drawAt(0.95, 0.5)], { delay: 10 });
      ...
      assertEqual(
        await evaluate("return window.__fixtureState.canvasDrawing"),
        false,
        "canvas drawing state resets after mouseup"
      );

      /* Verify pixels were drawn at three positions along the stroke path. */
      assert(await evaluate("return " + ${JSON.stringify(pixelDrawn(0.15, 0.5))}), "pixels drawn at stroke start region");
      assert(await evaluate("return " + ${JSON.stringify(pixelDrawn(0.5, 0.5))}), "pixels drawn at stroke midpoint");
      assert(await evaluate("return " + ${JSON.stringify(pixelDrawn(0.85, 0.5))}), "pixels drawn at stroke end region");

      /* Verify the last recorded point is near the end of the stroke. */
      const lastPoint = await evaluate("return window.__fixtureState.canvasLastPoint");
      const lastFrac = lastPoint.x / await evaluate("return document.querySelector('#draw-canvas').width");
  ...
  },
  {
    ...
    body: homeCase(`
      ...
      await drag([drawAt(0.1, 0.25), drawAt(0.5, 0.25), drawAt(0.9, 0.25)], { delay: 15 });
      await waitForJsValue("window.__fixtureState.canvasStrokes", 1, "first stroke registered");
      assert(await evaluate("return " + ${JSON.stringify(pixelDrawn(0.5, 0.25))}), "pixels drawn on first stroke path");

      /* Stroke 2: horizontal line in the lower third. */
      await waitForTimeout(100);
      await drag([drawAt(0.1, 0.75), drawAt(0.5, 0.75), drawAt(0.9, 0.75)], { delay: 15 });
      ...
      const secondStrokeFound = await evaluate(\`(() => {
        const c = document.querySelector("#draw-canvas");
        const ctx = c.getContext("2d");
      ...
      await waitForTimeout(100);
      await drag([drawAt(0.3, 0.1), drawAt(0.3, 0.5), drawAt(0.3, 0.9)], { delay: 15 });
      await waitForJsValue("window.__fixtureState.canvasStrokes", 3, "third stroke registered");
      assert(await evaluate("return " + ${JSON.stringify(pixelDrawn(0.3, 0.5))}), "pixels drawn on vertical stroke");
    `),
  },
  {
    ...
    body: homeCase(`
      ...
      assert(await evaluate("return " + ${JSON.stringify(pixelDrawn(0.08, 0.5))}), "pixels at zigzag start");
      assert(await evaluate("return " + ${JSON.stringify(pixelDrawn(0.3, 0.17))}), "pixels at zigzag peak 1");
      assert(await evaluate("return " + ${JSON.stringify(pixelDrawn(0.5, 0.83))}), "pixels at zigzag valley");
      assert(await evaluate("return " + ${JSON.stringify(pixelDrawn(0.75, 0.17))}), "pixels at zigzag peak 2");
      assert(await evaluate("return " + ${JSON.stringify(pixelDrawn(0.93, 0.5))}), "pixels at zigzag end");
