...
const FACADE_HELP: Record<string, string> = {
  ...
  locator:
    "page.locator(selector): returns an auto-waiting locator facade with locator(), getByRole(), getByText(), filter(), first(), nth(index), last(), click(), fill(value), clear(), press(key), check(), selectOption(value), textContent(), innerText(), innerHTML(), isVisible(), isEnabled(), getAttribute(name), screenshot(), count(), evaluate(fn, arg), evaluateAll(fn, arg), and waitFor(options). Unqualified raw CSS and raw xpath= selectors are not strict and act on the first match by default; prefer getBy*, loc=..., or chained/filtered locators when ambiguity must fail loudly. Narrow multiple matches; use first()/nth() only for confirmed legitimate duplicates.",
  browser:
